...
## (Plus + normal increase) to constantly add points to the left_2
...
## Just_collided is a flag that will deterine its first location, increase will add the rest  of the points
        if just_collided == false:
           x_pos = (%left_2.position.x + offset_x)
           y_pos = (%left_2.position.y + offset_y)
           just_collided = true
...
## this will decide the max_increase
    elif not %left_2.is_colliding() and just_collided == true:
            max_point()
