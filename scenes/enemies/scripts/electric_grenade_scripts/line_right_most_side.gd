...
## (Plus + normal increase) to constantly add points to the right
    point = Vector2((x_pos + normal_increase ), y_pos )
## Points outside the if statemnts cuz its an argument that constantly needs to be checked for duplicates
...
## Just_collided is a flag that will deterine its first location, increase will add the rest  of the points
        if just_collided == false:
           x_pos = (%up_down_right.position.x + offset_x)
           y_pos = (%up_down_right.position.y + offset_y)
           just_collided = true
...
## this will decide the max_increase
    elif not %up_down_right.is_colliding() and just_collided == true:
            max_point()
